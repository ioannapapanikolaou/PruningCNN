    ...
    def __init__(self, root, phase):
        ...
        if self.phase == 'train':
            self.transforms = T.Compose([T.Resize(224),T.RandomHorizontalFlip(),
                                    ...
                                    T.ColorJitter(brightness=0.5, hue=0.4),T.ToTensor(), T.Normalize([0.670, 0.464, 0.710], [0.146, 0.165, 0.111])]) # https://discuss.pytorch.org/t/normalization-image-for-using-pretrain-model/15348

        else:
            self.transforms = T.Compose([T.Resize(224),T.ToTensor(), T.Normalize([0.670, 0.464, 0.710], [0.146, 0.165, 0.111])])
